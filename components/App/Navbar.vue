<template>
  <div ref="headerRef" :style="styles" class="fixed top-0 w-full z-50">
    <nav class="mx-auto px-4 sm:px-6 lg:px-8 max-w-2xl">
      <ul
        class="flex items-center my-4 px-8 text-sm font-medium text-gray-800 shadow-lg bg-white/90 shadow-gray-800/5 ring-1 backdrop-blur dark:bg-gray-800/90 dark:text-gray-200 dark:ring-white/20 ring-gray-900/5"
      >
        <li v-for="item in items" :key="item.path">
          ...
          >
            <ULink
              :to="item.path"
              class="relative px-4 py-8 flex items-center justify-center transition hover:text-primary-500 dark:hover:text-primary-400"
            ...
            >
              ...
              <span
                v-if="$route.path === item.path"
                class="absolute h-8 w-8 z-0 rounded bg-gray-100 dark:bg-white/10 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"
              ></span>
